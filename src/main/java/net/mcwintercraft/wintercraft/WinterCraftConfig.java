...
public class WinterCraftConfig extends YamlConfiguration {

    protected static final Logger LOGGER = Logger.getLogger("WinterCraft");
    protected final File configFile;
    protected String templateName = null;
    protected static final Charset UTF8 = Charset.forName("UTF-8");
    private Class<?> resourceClass = WinterCraftConfig.class;
    private static final ExecutorService EXECUTOR_SERVICE = Executors.newSingleThreadExecutor();
    private final AtomicInteger pendingDiskWrites = new AtomicInteger(0);
    private final AtomicBoolean transaction = new AtomicBoolean(false);

    public WinterCraftConfig(final File configFile) {
        super();
    ...
    private void createFromTemplate() {
        ...
        try {
            ...
            try {
                ...
                Logger.getLogger(WinterCraftConfig.class.getName()).log(Level.SEVERE, null, ex);
            }
            try {
                if (ostr != null) {
                    ostr.close();
    ...
    private Future<?> delayedSave(final File file) {
        ...
        return EXECUTOR_SERVICE.submit(new WinterCraftConfig.WriteRunner(configFile, data, pendingDiskWrites));
    }
    ...
    @Override
    public synchronized org.bukkit.util.Vector getVector(String path) {
        return super.getVector(path);
    }

    @Override
    public synchronized org.bukkit.util.Vector getVector(String path, org.bukkit.util.Vector def) {
